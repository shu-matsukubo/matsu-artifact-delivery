...
成果物を利用者へ届けるデリバリーを支援するプラグイン集です。現在の `artifact-workflow` は、その一部である成果物生成フローを担当します。生成・品質確認を終えた後の更新・公開・提出などは、親エージェントが担当 Plugin・Skill の責務として続行します。[Agent Plugins](https://agent-plugins.org/) の共通規格を基準とし、当面は Codex を主な実行環境とします。

## Plugin 一覧

| Plugin | 目的 |
| --- | --- |
| [artifact-workflow](plugins/artifact-workflow/README.md) | 成果物を作る作業タスクを計画・承認し、生成・セルフレビュー・タスクと全体の検証を行う。完成品と検証結果を提示・引き渡してフローを終了し、後続 Plugin の処理は管理しない。 |
