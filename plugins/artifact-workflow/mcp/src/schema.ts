...
export const sessionIdSchema = z.string().regex(/^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$/)
...
export const planSchema = z.strictObject({
  ...
  if (plan.tasks.some(task => !visit(task.id))) {
    ctx.addIssue({ code: 'custom', path: ['tasks'], message: 'Dependencies must name existing tasks and have no cycles.' });
  }
});
