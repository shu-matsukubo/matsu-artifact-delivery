...
自然言語部分の試験データは [Workflow 契約一覧](../test/workflow/contracts.json)と [Escalation 契約一覧](../test/escalation/contracts.json)。安定 ID・観点名・対象ファイル・必要な契約文を持ち、実行結果にも ID を表示する。構造試験は全 Skill の Markdown と全 Agent 定義に契約項目があることを要求する。新しい実装形式を Skill 配下へ追加するときも、対応する単体試験を追加する。
...
| WF-E04 / WF-E05          | Escalation 未導入で必須参照が切れない、フォールバック契約、参照切れ・権限退行の検出         | Workflow E2E                                          |
| EX-E01 / EX-E02 / EX-E03 | Escalation 単独配布、両相談役とテンプレートの識別子・状態整合、参照欠落の検出               | [Escalation E2E](../test/escalation/e2e.test.mjs)     |
